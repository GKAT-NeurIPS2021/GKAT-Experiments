...
class GWK_loop(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.device == 'cpu':
            ...
            #for c in range(len(counting_attn)):
            #    counting_attn[c] = counting_attn[c].cuda()
        ...
        # mini_batch = (batch.max().tolist()+1) / self.mini_batch_size
        
        
        for bt in range(batch.max().tolist()+1):
            if self.device == 'cpu':
                x_bt = x[torch.where(batch==bt)[0]]
                counting_attn_bt = counting_attn[bt].type(torch.FloatTensor)
            elif self.device == 'cuda':
                x_bt = x[torch.where(batch==bt)[0]].cuda()
                counting_attn_bt = counting_attn[bt].type(torch.FloatTensor).cuda()  
            ...
            for i, gnn in enumerate(self.layers):
                ...
                for j, att_head in enumerate(gnn):
                    all_h.append(att_head(x_bt, counting_attn_bt))   
                x_bt = torch.squeeze(torch.cat(all_h, dim=1))  
            
            if x_out==None:
                if self.pooling == 'max':
                    x_out = x_bt.max(0)[0][None,:]
                elif self.pooling == 'mean':
                    x_out = x_bt.sum(0)[None,:]

            else:
                if self.pooling == 'max':
                    x_out = torch.cat((x_out,x_bt.max(0)[0][None,:]), dim = 0)
                elif self.pooling == 'mean':
                    x_out = torch.cat((x_out,x_bt.sum(0)[None,:]), dim = 0)
