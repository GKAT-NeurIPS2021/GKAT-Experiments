...
class GWK_full(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.batch_forloop:
            ...
            for bt in range(batch.max().tolist()+1):
                ...
                if x_out==None:
                    if self.pooling == 'max':
                        ...
                        #elif self.device == 'cuda':
                        #    x_out = x_bt.max(0)[0][None,:].cuda()
                    elif self.pooling == 'mean':
                        #if self.device == 'cpu':
                        x_out = x_bt.sum(0)[None,:]
                        #elif self.device == 'cuda':
                        #    x_out = x_bt.sum(0)[None,:].cuda()
                else:
                    if self.pooling == 'max':
            ...
            if self.pooling == 'max':
                x_out = global_max_pool(x_bt, batch)
            elif self.pooling == 'mean':
                x_out = global_add_pool(x_bt, batch)
