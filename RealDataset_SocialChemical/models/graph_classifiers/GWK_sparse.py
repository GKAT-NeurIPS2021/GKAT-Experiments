...
class GWKLayer_exp_sparse(nn.Module):
    def __init__(self, in_dim, out_dim, feat_drop=0., attn_drop=0., alpha=0.2, agg_activation=F.elu):
        super(GWKLayer_exp_sparse, self).__init__()
...
class GWK_sparse(nn.Module):
    def __init__(self, dim_features, dim_target, config):
        ...
        if len(self.hidden_dim)==0:
            if config['concat_exp'] == 'concat':
                self.layers.append( GWKLayer_sparse(self.dim_mid[0], self.dim_mid[0], feat_drop = self.feat_drop, attn_drop = self.attn_drop, agg_activation=self.agg_activation) )
            elif config['concat_exp'] == 'exp':
                self.layers.append( GWKLayer_exp_sparse(self.dim_mid[0], self.dim_mid[0], feat_drop = self.feat_drop, attn_drop = self.attn_drop, agg_activation=self.agg_activation) )
        else:
            ...
            for i in range(len(self.hidden_dim)):
                ...
                for _ in range(num_heads_out):
                    if config['concat_exp'] == 'concat':
                        h_list.append( GWKLayer_sparse(hid_dim_in * num_heads_in , hid_dim_out , feat_drop = self.feat_drop, attn_drop = self.attn_drop, agg_activation=self.agg_activation) )
                    elif config['concat_exp'] == 'exp':
                        h_list.append( GWKLayer_exp_sparse(hid_dim_in * num_heads_in , hid_dim_out , feat_drop = self.feat_drop, attn_drop = self.attn_drop, agg_activation=self.agg_activation) )
            ...
            if config['concat_exp'] == 'concat':
                self.layers.append( nn.ModuleList([GWKLayer_sparse(hid_dim_in * num_heads_in , hid_dim_out * num_heads_out, feat_drop = self.feat_drop, attn_drop = self.attn_drop, agg_activation=self.agg_activation)] ) )    
            elif config['concat_exp'] == 'exp':
                self.layers.append( nn.ModuleList([GWKLayer_exp_sparse(hid_dim_in * num_heads_in , hid_dim_out * num_heads_out, feat_drop = self.feat_drop, attn_drop = self.attn_drop, agg_activation=self.agg_activation)] ) )    
    ...
    def forward(self, data):
        ...
        torch.save(data.batch, 'batch.pt')
        #torch.save(counting_attn, 'counting_attn.pt')
        torch.save(data.counting_attn_size, 'counting_attn_size.pt')
        torch.save(data.counting_attn_values, 'counting_attn_values.pt')
        torch.save(data.counting_attn_indices, 'counting_attn_indices.pt')
        
        
        if self.device == 'cpu':
            x, batch, counting_attn_size, counting_attn_indices, counting_attn_values = data.x, data.batch, data.counting_attn_size, data.counting_attn_indices, data.counting_attn_values
        elif self.device == 'cuda':
            x, batch, counting_attn_size, counting_attn_indices, counting_attn_values = data.x.cuda(), data.batch.cuda(), data.counting_attn_size, data.counting_attn_indices, data.counting_attn_values
        ...
        counting_attn_size = torch.load('counting_attn_size.pt')
        counting_attn_values = torch.load('counting_attn_values.pt')
        counting_attn_indices = torch.load('counting_attn_indices.pt')
        ...
        ##### change indices of sparse 
        last_size = counting_attn_size[0][0]
        for b in range(1, 32):#self.batch_size):
            counting_attn_indices[b] += last_size
            last_size += counting_attn_size[b][0]  
        
        sparse_indices = torch.transpose(torch.cat(counting_attn_indices),1,0)
        sparse_values = torch.cat(counting_attn_values)
        
        couting_attn_sparse = torch.sparse_coo_tensor(sparse_indices, sparse_values.squeeze(), (last_size, last_size))
        ...
        for i, gnn in enumerate(self.layers):
            ...
            for j, att_head in enumerate(gnn):
                all_h.append(att_head(x, counting_attn_sparse))   
